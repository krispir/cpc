...
colorRampPalette(colors = c("#264653", "#2A9D8F", "#E9C46A", 
                            "#F4A261", "#E76F51"))(16)

plot(1:16, rep(1, 16), pch = 20, cex = 5, 
     col = colorRampPalette(colors = c("#264653", "#2A9D8F", "#E9C46A", 
                                       "#F4A261", "#E76F51"))(16))

snVals <- cpcPeaktable$sn
snVals[which(snVals == Inf)] <- max(snVals[which(snVals != Inf)])*2
seq(from = 0, to = max(snVals), by = max(snVals)/16)
...
plot(x = xcmsPeaktableOrig$rt[-removedPeaksId], 
     y = xcmsPeaktableOrig$mz[-removedPeaksId], pch = 20, 
     col = colVec[-removedPeaksId],
     cex = log(pmax(0, xcmsPeaktableOrig$into[-removedPeaksId]) / 
                 median(pmax(0, xcmsPeaktableOrig$into[-removedPeaksId]))),
...
layout(mat = matrix(c(1,2), nrow = 1), widths = c(0.53,0.47))
par(mar = c(5.1,4.1,2.1,0.1))
plot(x = cpcPeaktable$apex[-removedPeaksId], 
     y = xcmsPeaktableOrig$mz[-removedPeaksId], pch = 20, 
     col = colVec[-removedPeaksId],
     cex = log(pmax(0, cpcPeaktable$area[-removedPeaksId]) / 
                 median(pmax(0, cpcPeaktable$area[-removedPeaksId]))),
     xlab = "Scan index", ylab = "m/z", main = "Retained peaks")
...
plot(x = cpcPeaktable$apex[removedPeaksId], 
     ...
     cex = log(pmax(0, cpcPeaktable$area[removedPeaksId]) / 
                 median(pmax(0, cpcPeaktable$area[removedPeaksId]))),
     xlab = "Scan index", yaxt = "n", ylab = "", main = "Removed peaks")
...
seed <- c(78, 3.48167, -2.30259, 14.0392, 
          98, 6.55711, -2.30259, 15.6597, 
          112, 2.525, -2.30259, 14.3057, 
          127, 1.90602, -2.30259, 12.5534, 
          140, 4.37691, -2.30259, 13.5071)

lbounds <- c(58, 1.74084, -9.21034, 13.3461, 
             78, 3.27856, -9.21034, 14.9666, 
             92, 1.2625, -9.21034, 13.6125, 
             107, 0.953009, -9.21034, 11.8602, 
             120, 2.18846, -9.21034, 12.8139)

ubounds <- c(98, 5.22251, 2.99573, 14.4447, 
             118, 9.83567, 2.99573, 16.0652, 
             132, 3.7875, 2.99573, 14.7111, 
             147, 2.85903, 2.99573, 12.9588, 
             160, 6.56537, 2.99573, 13.9126)
...
# sink(file = paste0(getwd(), "/loc_data/210523_deconv_example_3.txt"),
#      type = "output", split = T)
# cpcParam <- cpcProcParam(fit_emg = TRUE, sel_peaks = 34, plot = F,
#                          verbose_output = T)
# cpcDeconv <- characterize_xcms_peaklist(xd = xd, param = cpcParam)
# sink(file = NULL)
