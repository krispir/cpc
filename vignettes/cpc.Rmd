...
xcmsPeaktableOrig <- cpc::getPeaklist(cpc)
...
So in the original XCMS object there were 1110 peaks identified in 4 files. In the filtered object there are 852 peaks identified, meaning 258 peaks were removed.
...
rbind(Original = table(chromPeaks(xdOrig)[, "sample"]),
      Filtered = table(chromPeaks(xdFilt)[, "sample"]),
      Difference = table(chromPeaks(xdOrig)[, "sample"]) - 
        table(chromPeaks(xdFilt)[, "sample"]))

```

So 52-71 peaks per sample were removed by the algorithm in each file. Let's have a look at some of these removed peaks. We will randomly select 2 peaks from each reson for removal.
...
# (3) Too narrow
which(cpcPeaktable$note[removedPeaksId] == "not_detected")

# select 5 of the removed peaks at random
set.seed(seed); (randomRemoved <- sample(x = removedPeaks, size = 5, replace = F))
