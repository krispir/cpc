...
// Get the highest, second highest, and lowest vertices in the simplex
// This will be done by looping over the simplex vertices
// Note: This is replaced by the sort() method
// void Simplex::find_extreme_vertices()
// {
//   this->ilo = 0;
//   this->ihi = 0;
//   this->inhi = 1;
//   
//   for (unsigned int i = 0; i < this->npar+1; i++)
//   {
//     if (this->verts.at(i).fval > this->verts.at(this->ihi).fval)
//     {
//       this->ihi = i;
//     } else if (this->verts.at(i).fval < this->verts.at(this->ihi).fval && 
//                this->verts.at(i).fval > this->verts.at(this->inhi).fval)
//     {
//       this->inhi = i;
//     } else if (this->verts.at(i).fval < this->verts.at(this->ilo).fval)
//     {
//       this->ilo = i;
//     }
//   }
...
void Minimizer<Derived>::nmmin(Derived &fn_)
{
    ...
    if (this->control.trace)
    {
        ...
        if (this->tfval)
        {
            Rcout << "Terminated by function value tolerance." << std::endl;
        } else if (this->tvert)
        {
            Rcout << "Terminated by simplex size tolerance." << std::endl;
        } else if (this->titer)
        {
            Rcout << "Maximum iterations reached." << std::endl;
        } else if (this->tfval)
        {
            Rcout << "Maximum function evaluations reached." << std::endl;
        }
    }
}
...
//         if (this->options.output)
//         {
//             Rcout << std::endl << "Fitting cluster: " << i << std::endl;
//         }
//         
//         // clear the scan vector
//         scans_to_fit.clear();
//         
//         // get the current cluster
//         cur_clust = this->pt.final_clust.at(i);
//         
//         // determine which peaks in the cluster to fit using relative size limit
//         // any peak with an apex value < this->options.rel_fit_lim * largest peak apex val
//         // will not be fitted
...
//             if (j == 0)
//             {
//                 // cur_scans = LinearSpacedArray(this->pt.fpkb.at(cur_clust.at(0)+j),
//                 //                               this->pt.tpkb.at(cur_clust.at(0)+j),
//                 //                               this->options.pts_per_peak);
//                 cur_scans = LinearSpacedArray(this->pt.fpkb.at(peaks_to_fit.at(j)),
//                                               this->pt.tpkb.at(peaks_to_fit.at(j)),
//                                               this->options.pts_per_peak);
//                 
//                 scans_to_fit.insert(scans_to_fit.end(), cur_scans.begin(), cur_scans.end());
//             } else
//             {
//                 // cur_scans = LinearSpacedArray(this->pt.fpkb.at(cur_clust.at(0)+j)+1,
//                 //                               this->pt.tpkb.at(cur_clust.at(0)+j),
//                 //                               this->options.pts_per_peak);
//                 cur_scans = LinearSpacedArray(this->pt.fpkb.at(peaks_to_fit.at(j))+1,
//                                               this->pt.tpkb.at(peaks_to_fit.at(j)),
//                                               this->options.pts_per_peak);
//                 
//                 scans_to_fit.insert(scans_to_fit.end(), cur_scans.begin(), cur_scans.end());
...
//         
//         // create data vectors for minimizer
//         arma::vec st(n_scans_to_fit); // scan index (time)
//         arma::vec si(n_scans_to_fit); // scan intensity
//         arma::vec wt(n_scans_to_fit); // weights
//         arma::vec seed(n_peaks_to_fit*4); // seed params
//         arma::vec lower(n_peaks_to_fit*4); // lower bounds for params
//         arma::vec upper(n_peaks_to_fit*4); // upper bounds for params
//         
//         // populate the vectors
//         for (j = 0; j < n_scans_to_fit; j++)
//         {
//             st.at(j) = scans_to_fit.at(j);
//             si.at(j) = this->d0.at(scans_to_fit.at(j));
//             wt.at(j) = 1.0;
//         }
//         
//         for (j = 0; j < n_peaks_to_fit; j++)
//         {
//             // seed.at(j*4) = (double) this->pt.adj_apex.at(cur_clust.at(0)+j); // mu
//             // seed.at(1+(j*4)) = (double) (this->pt.tinf.at(cur_clust.at(0)+j) -
//             //     this->pt.finf.at(cur_clust.at(0)+j)) / 2; // sigma
//             // seed.at(2+(j*4)) = log(0.5); // lambda
//             // // seed.at(3+(i*4)) = 100.0; // area (need better init value)
//             // // TODO: calculate area by relationship between max value and
//             // //       sigma? A = H*sigma/0.3989
//             // // seed.at(3+(i*4)) = log((double)
//             // //     this->d0.at(this->pt.adj_apex.at(cur_clust.at(0)+i)) *
//             // //         seed.at(1+(i*4)) / 0.3989); // quick and (maybe) ugly area est.
//             // seed.at(3+(j*4)) = this->integrate_d0((int) this->pt.fpkb.at(cur_clust.at(0)+j),
//             //         (int) this->pt.tpkb.at(cur_clust.at(0)+j));
//             seed.at(j*4) = (double) this->pt.adj_apex.at(peaks_to_fit.at(j)); // mu
//             seed.at(1+(j*4)) = (double) (this->pt.tinf.at(peaks_to_fit.at(j)) -
//                 this->pt.finf.at(peaks_to_fit.at(j))) / 2; // sigma
//             seed.at(2+(j*4)) = log(0.5); // lambda
...
//             upper.at(j*4) = seed.at(j*4) + 20; // upper mu
//             upper.at(1+(j*4)) = seed.at(1+(j*4)) * 1.5; // upper sigma
//             upper.at(2+(j*4)) = std::log(30); // upper lambda
//             upper.at(3+(j*4)) = std::log(seed.at(3+(j*4)) * 1.5); // upper area (infinite)
//             
//             seed.at(3+(j*4)) = log(seed.at(3+(j*4))); // scale the area
//         }
//         
//         if (this->options.output)
//         {
...
//                 Rcout << left << setw(colwidth) << setfill(filler) << seed.at((j*4)+0);
//                 Rcout << left << setw(colwidth) << setfill(filler) << seed.at((j*4)+1);
//                 Rcout << left << setw(colwidth) << setfill(filler) << exp(seed.at((j*4)+2));
//                 Rcout << left << setw(colwidth) << setfill(filler) << exp(seed.at((j*4)+3));
//                 
//                 if (this->get_vip() == cur_clust.at(0)+j)
//                 {
//                     Rcout << left << setw(colwidth) << setfill(filler) << "*";
...
//         //        double _h = 1e-5,
//         //        const std::string &_method = "Not-NM")
//         EMGFit fit_fun(si, st, wt, seed, lower, upper, n_peaks_to_fit,
//                        1e-4, this->options.fit_method);
//         
//         this->fit = Roptim<EMGFit>(this->options.fit_method);
//         
//         if (this->options.output)
//         {
//             this->fit.control.trace = 1;
//         } else
//         {
//             this->fit.control.trace = 0;
//         }
//         
//         this->fit.set_hessian(this->options.fit_hess);
//         
//         if (this->options.fit_method == "L-BFGS-B")
//         {
//             this->fit.set_lower(lower);
//             this->fit.set_upper(upper);
//         }
//         
//         this->fit.control.reltol = this->options.reltol;
//         this->fit.control.alpha = this->options.alpha;
//         this->fit.control.beta = this->options.beta;
//         this->fit.control.gamma = this->options.gamma;
//         // this->fit.control.maxit = this->options.maxit;
//         this->fit.minimize(fit_fun, seed);
//         
//         arma::vec opt_param = this->fit.par();
//         
//         // record values
//         for (j = 0; j < n_peaks_to_fit; j++)
//         {
...
//     
//     // 
//     // // check if only vip is to be fitted and that vip is set
//     // if (this->options.fit_only_vip && this->get_vip() >= 0)
//     // {
//     //     // get the cluster that contains the vip and use this to determine
//     //     // which peaks should be fitted along with the vip
//     //     i = 0;
//     //     while(i < this->pt.final_clust.size() && 
//     //           !(this->pt.final_clust.at(i).at(0) <= this->get_vip() &&
//     //             this->pt.final_clust.at(i).at(1) >= this->get_vip()))
...
//     //     }
//     //     
//     //     // ensure we arrived at a matching cluster - else just leave the
//     //     // fitted values at -1
//     //     // TODO: initialize the fitted values as -1
//     //     if (this->pt.final_clust.at(i).at(0) <= this->get_vip() &&
//     //         this->pt.final_clust.at(i).at(1) >= this->get_vip())
//     //     {
//     //         cur_clust_idx = i;
//     //         cur_clust = this->pt.final_clust.at(cur_clust_idx);
//     //         
//     //         if (this->options.output)
//     //         {
//     //             Rcout << "Peaks in cluster: " << cur_clust.at(0)
//     //                   << "->" << cur_clust.at(1) << std::endl;
//     //         }
//     //         
//     //         // determine which peaks for fit along with the vip
//     //         // Strategy #1:
//     //         //  - fit entire cluster
//     //         // 
//     //         // Strategy #2:
//     //         //  - any peak with a resolution < 1.0 to the vip
//     //         // 
//     //         // Strategy #3:
//     //         //  - the peak before and after the vip
//     //         //  - if any of the peaks before and after is a shoulder peak
//     //         //      - the peak after the shoulder peak if it has a Rs < 1.0 to
//     //         //        the shoulder peak
//     //         
//     //         // strategy #1: fit entire cluster
//     //         n_peaks_to_fit = cur_clust.at(1) - cur_clust.at(0) + 1;
//     //         
//     //         if (this->options.output)
//     //         {
//     //             Rcout << "No. peaks to fit: " << n_peaks_to_fit << std::endl;
//     //         }
//     //         
//     //         // vec_i peaks_to_fit(no_peaks_to_fit);
//     //         // std::iota(peaks_to_fit.begin(), peaks_to_fit.end(), cur_clust.at(0));
//     //         
//     //         // strategy #2: peaks with resolution < 1.0 to vip
//     //         // TODO!
//     //         
//     //         // strategy #3: directly adjacent peaks
//     //         // TODO!
//     //         
//     //         
//     //         
//     //         // determine scans to fit by peak
//     //         n_scans_to_fit = 0;
//     //         // Strategy #1:
//     //         //  - fit the entire range between the baseline bounds of the cluster
//     //         // 
//     //         // Strategy #2:
//     //         //  - fit between peak bounds of each peak
//     //         // 
//     //         // Strategy #3:
//     //         //  - fit between inflection points of each peak
//     //         
//     //         // strategy #1: entire baseline range of cluster
//     //         // TODO!
//     //         
//     //         // strategy #2: between peak bounds of each peak (same as #1 if 
//     //         // entire cluster is fitted)
//     //         // std::vector<vec_i> scans_to_fit_by_peak(no_peaks_to_fit, vec_i(2));
//     //         // 
//     //         // if (no_peaks_to_fit == 1) // if only 1 peak in cluster
//     //         // {
//     //         //     scans_to_fit_by_peak.at(0) = { (int) this->pt.fpkb.at(cur_clust.at(0)),
//     //         //                                    (int) this->pt.tpkb.at(cur_clust.at(0)) };
//     //         // 
//     //         //     no_scans_to_fit = scans_to_fit_by_peak.at(0).at(1) - scans_to_fit_by_peak.at(0).at(0) + 1;
//     //         // 
//     //         // } else // if multiple peaks in the cluster
//     //         // {
//     //         //     for (i = 0; i < no_peaks_to_fit; i++)
//     //         //     {
//     //         //         if (i == 0) // if first peak in cluster
//     //         //         {
//     //         //             scans_to_fit_by_peak.at(i) = { (int) this->pt.fpkb.at(cur_clust.at(i)),
//     //         //                                            (int) this->pt.tpkb.at(cur_clust.at(i)) };
//     //         // 
//     //         //         } else // if not first peak in cluster
//     //         //         {
//     //         //             // if front bound of current peak is same or lower than
//     //         //             // the tail bound of last peak in cluster
//     //         //             if (this->pt.fpkb.at(cur_clust.at(i)) <=
//     //         //                 this->pt.tpkb.at(cur_clust.at(i-1)))
//     //         //             {
//     //         //                 scans_to_fit_by_peak.at(i) = { (int) this->pt.tpkb.at(cur_clust.at(i)) -
//     //         //                                                      this->pt.fpkb.at(cur_clust.at(i-1)),
//     //         //                                                (int) this->pt.tpkb.at(cur_clust.at(i)) };
//     //         // 
//     //         //             } else // if front bound of current peak is larger than
//     //         //                    // the last peak in cluster
//     //         //             {
//     //         //                 scans_to_fit_by_peak.at(i) = { (int) this->pt.fpkb.at(cur_clust.at(i)),
//     //         //                                                (int) this->pt.tpkb.at(cur_clust.at(i)) };
//     //         //             }
//     //         // 
//     //         //         } // which peak in cluster
//     //         // 
//     //         //         no_scans_to_fit += scans_to_fit_by_peak.at(i).at(1) - scans_to_fit_by_peak.at(i).at(0) + 1;
//     //         // 
//     //         //     } // loop over cluster
//     //         // 
//     //         // } // if one or more peaks
//     //         // 
//     //         // if (this->options.output)
//     //         // {
//     //         //     Rcout << "Scans to fit: " << no_scans_to_fit << std::endl;
//     //         //     Rcout << "i\ts\te" << std::endl;
//     //         //     
//     //         //     for (i = 0; i < no_peaks_to_fit; i++)
//     //         //     {
//     //         //         Rcout << i << "\t";
//     //         //         for (j = 0; j < scans_to_fit_by_peak.at(i).size(); j++)
//     //         //         {
//     //         //             Rcout << scans_to_fit_by_peak.at(i).at(j) << "\t";
//     //         //         }
//     //         //         Rcout << std::endl;
//     //         //     }
//     //         // }
//     //         
//     //         // strategy #3: between inflection points of each peak
//     //         std::vector<vec_i> scans_to_fit_by_peak(n_peaks_to_fit, vec_i(2));
//     //         
//     //         for (int i = 0; i < n_peaks_to_fit; i++)
//     //         {
//     //             if (i > 0 && 
//     //                 this->pt.fpkb.at(cur_clust.at(0)+i) == this->pt.fpkb.at(cur_clust.at(0)+(i-1)))
//     //             {
//     //                 // scans_to_fit_by_peak.at(i) = 
//     //                 //     { (int) std::floor(this->pt.finf.at(cur_clust.at(0)+i)+0.5),
//     //                 //       (int) std::floor(this->pt.tinf.at(cur_clust.at(0)+i)+0.5),
//     //                 //       (int) this->pt.tpkb.at(cur_clust.at(0)+i)};
//     //                 scans_to_fit_by_peak.at(i) = 
//     //                     { (int) this->pt.fpkb.at(cur_clust.at(0)+i)+1,
//     //                       (int) this->pt.tpkb.at(cur_clust.at(0)+i) };
//     //             } else
//     //             {
//     //                 scans_to_fit_by_peak.at(i) = 
//     //                     { (int) this->pt.fpkb.at(cur_clust.at(0)+i),
//     //                       (int) this->pt.tpkb.at(cur_clust.at(0)+i) };
//     //             }
//     //             
//     //             n_scans_to_fit += scans_to_fit_by_peak.at(i).at(1) - 
//     //                 scans_to_fit_by_peak.at(i).at(0)+1;
//     //         }
//     //         
//     //         if (this->options.output)
//     //         {
//     //             Rcout << "Scans to fit: " << n_scans_to_fit 
//     //                   << std::endl << std::endl;
//     //             
//     //             Rcout << left << setw(5) << setfill(filler) << "peak";
//     //             Rcout << left << setw(colwidth) << setfill(filler) << "start";
//     //             Rcout << left << setw(colwidth) << setfill(filler) << "end";
//     //             Rcout << std::endl;
//     // 
//     //             for (i = 0; i < n_peaks_to_fit; i++)
//     //             {
//     //                 Rcout << left << setw(5) << setfill(filler) << i;
//     //                 
//     //                 for (j = 0; j < scans_to_fit_by_peak.at(i).size(); j++)
//     //                 {
//     //                     Rcout << left << setw(colwidth) << setfill(filler)
//     //                           << scans_to_fit_by_peak.at(i).at(j);
//     //                 }
//     //                 
//     //                 Rcout << std::endl;
//     //             }
...
//     //         }
//     //         
//     //         // create vectors to hold data points
//     //         arma::vec st(n_scans_to_fit); // scan index (time)
//     //         arma::vec si(n_scans_to_fit); // scan intensity
//     //         arma::vec wt(n_scans_to_fit); // weights
//     //         arma::vec seed(n_peaks_to_fit*4); // seed params
...
//     //         for (int i = 0; i < n_peaks_to_fit; i++)
//     //         {
//     //             seed.at(i*4) = (double) this->pt.adj_apex.at(cur_clust.at(0)+i); // mu
//     //             seed.at(1+(i*4)) = (double) (this->pt.tinf.at(cur_clust.at(0)+i) -
//     //                                 this->pt.finf.at(cur_clust.at(0)+i)) / 2; // sigma
//     //             seed.at(2+(i*4)) = log(0.5); // lambda
//     //             // seed.at(3+(i*4)) = 100.0; // area (need better init value)
//     //             // TODO: calculate area by relationship between max value and
//     //             //       sigma? A = H*sigma/0.3989
//     //             // seed.at(3+(i*4)) = log((double)
//     //             //     this->d0.at(this->pt.adj_apex.at(cur_clust.at(0)+i)) *
//     //             //         seed.at(1+(i*4)) / 0.3989); // quick and (maybe) ugly area est.
//     //             seed.at(3+(i*4)) = this->integrate_d0((int) this->pt.fpkb.at(cur_clust.at(0)+i),
//     //                                                   (int) this->pt.tpkb.at(cur_clust.at(0)+i));
//     //             
//     //             // set lower bounds
//     //             lower.at(i*4) = seed.at(i*4) - 20; // lower mu
//     //             lower.at(1+(i*4)) = seed.at(1+(i*4)) * 0.5; // lower sigma
//     //             lower.at(2+(i*4)) = std::log(1e-4); // lower lambda
//     //             lower.at(3+(i*4)) = std::log(seed.at(3+(i*4)) * 0.5); // lower area
//     // 
//     //             // set upper bounds
//     //             upper.at(i*4) = seed.at(i*4) + 20; // upper mu
//     //             upper.at(1+(i*4)) = seed.at(1+(i*4)) * 1.5; // upper sigma
//     //             upper.at(2+(i*4)) = std::log(30); // upper lambda
//     //             upper.at(3+(i*4)) = std::log(seed.at(3+(i*4)) * 1.5); // upper area (infinite)
//     //             
//     //             seed.at(3+(i*4)) = log(seed.at(3+(i*4))); // scale the area
//     //             
//     //             // populate the scans to fit
//     //             for (int j = scans_to_fit_by_peak.at(i).at(0);
//     //                  j <= scans_to_fit_by_peak.at(i).at(1); j++)
//     //             {
//     //                 st.at(k) = (double) j;
//     //                 si.at(k) = this->d0.at(j);
//     //                 wt.at(k) = 1.0;
//     //                 k++;
//     //             }
//     //         }
...
//     //             Rcout << left << setw(colwidth) << setfill(filler) << "vip" << std::endl;
//     //             
//     //             for (unsigned int i = 0; i < n_peaks_to_fit; i++)
//     //             {
//     //                 Rcout << left << setw(5) << setfill(filler) << cur_clust.at(0)+i;
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << seed.at((i*4)+0);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << seed.at((i*4)+1);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << exp(seed.at((i*4)+2));
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << exp(seed.at((i*4)+3));
//     //                 
//     //                 if (this->get_vip() == i)
//     //                 {
//     //                     Rcout << left << setw(colwidth) << setfill(filler) << "*";
...
//     //             {
//     //                 Rcout << left << setw(5) << setfill(filler) << cur_clust.at(0)+i;
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << lower.at((i*4)+0);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << lower.at((i*4)+1);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << lower.at((i*4)+2);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << lower.at((i*4)+3);
//     //                 
//     //                 if (this->get_vip() == i)
//     //                 {
//     //                     Rcout << left << setw(colwidth) << setfill(filler) << "*";
...
//     //             {
//     //                 Rcout << left << setw(5) << setfill(filler) << cur_clust.at(0)+i;
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << upper.at((i*4)+0);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << upper.at((i*4)+1);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << upper.at((i*4)+2);
//     //                 Rcout << left << setw(colwidth) << setfill(filler) << upper.at((i*4)+3);
//     //                 
//     //                 if (this->get_vip() == i)
//     //                 {
//     //                     Rcout << left << setw(colwidth) << setfill(filler) << "*";
...
//     //         for (int i = 0; i < n_peaks_to_fit; i++)
//     //         {
//     //             this->pt.emg_mu.at(cur_clust.at(0)+i) = opt_param.at(i*4);
//     //             this->pt.emg_sigma.at(cur_clust.at(0)+i) = opt_param.at(1+(i*4));
//     //             this->pt.emg_lambda.at(cur_clust.at(0)+i) = exp(opt_param.at(2+(i*4)));
//     //             this->pt.emg_area.at(cur_clust.at(0)+i) = exp(opt_param.at(3+(i*4)));
//     //             this->pt.emg_conv.at(cur_clust.at(0)+i) = this->fit.convergence();
//     //         }
//     //         
//     //     } // else -> leave values as initialized (-1.0)
//     //     
//     // } else // TODO: fit all clusters
//     // {
//     //     // fit all peaks
//     //     
//     //     unsigned int nclust = this->pt.final_clust.size();
//     //     
//     //     // loop over all clusters
//     //     if (nclust > 0)
//     //     {
//     //         // determine which scans to fit in the cluster
//     //         
//     //         
//     //         // fit all peaks in the cluster
//     //         
//     //         
